# Full-window results: PFML and mothers' labour supply, CPS ASEC 1990-2025
...
`pfml/python/03_build_ipums_asec.py`; estimated by `04_csdid_full.py` through
`05_run_full.sh`; assembled by `06_summarise_full.py`. Tables are in
`pfml/output/tables/full_*.csv|md`, figures in `pfml/output/figures/full_*.png`.
...
** 5%, *** 1%. The Stata `csdid` pipeline (`pfml/stata/`) runs the same
...
![event](../output/figures/full_event_mothers_vs_childless.png)
...
![group](../output/figures/full_group_worked.png)
...
![raw](../output/figures/full_raw_trends_worked.png)
