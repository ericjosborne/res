...
* **Policy panel:** `pfml/data/raw/pfml_policy_dates.csv`, enactment,
  ...
  state statutes.
* **Preliminary demonstration (this repository):** CPS ASEC 2021-2023 person
  records redistributed by PolicyEngine, reference years 2020-2022, women aged
  18-44 with linked own children (92 percent exact-match retention). In this
  window Massachusetts (2021) and Connecticut (2022) are treated cohorts,
  earlier adopters have no pre-period and are dropped, later adopters are
...
preliminary run exists to prove the pipeline end to end and to show what the
placebo and triple-difference logic does in practice. See
