...
```
pfml/data/raw/ipums_cps_asec.csv.gz      # the data
pfml/data/raw/ipums_cps_asec.xml         # the DDI codebook
```

Then run `python pfml/python/03_build_ipums_asec.py` and
`python pfml/python/04_csdid_full.py`, or in Stata `do pfml/stata/00_master.do`.
The Stata build reads the uncompressed CSV; gunzip it first
(`gunzip -k pfml/data/raw/ipums_cps_asec.csv.gz`) — the `.csv` is git-ignored.
...
`pfml/data/raw/*.csv.gz` or split by year (`zcat file.csv.gz | awk ...`) into
`pfml/data/raw/monthly/ipums_cps_monthly_YYYY.csv.gz`. The build script
