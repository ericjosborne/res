...
for s in mothers_lt6 childless mothers; do
  for y in worked hours fulltime inlf; do
    python3 pfml/python/04_csdid_full.py --sample $s --outcome $y --B $B --window -10 10 > pfml/output/logs/full_${s}_${y}.log 2>&1 &
    jobs+=($!)
  done
done
# not-yet-treated controls for the headline
python3 pfml/python/04_csdid_full.py --sample mothers_lt6 --outcome worked --notyet --B $B --window -10 10 > pfml/output/logs/full_mothers_lt6_worked_notyet.log 2>&1 &
jobs+=($!)
fail=0
