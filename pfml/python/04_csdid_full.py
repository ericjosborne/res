"""
04_csdid_full.py -- Callaway & Sant'Anna (2021) on any window / cohort set.

Runs on pfml/data/clean/cps_asec_women_1844.csv.gz (IPUMS extract, once it
is in the repository) and falls back to the PolicyEngine 2020-22 file, on
which it reproduces 02_csdid_prelim.py exactly.
...
Usage: python 04_csdid_full.py [--sample mothers_lt6|mothers|childless|all]
...
def load():
    p = CLEAN / "cps_asec_women_1844.csv.gz"
    if p.exists():
        return pd.read_csv(p), "ipums"
    return pd.read_csv(CLEAN / "cps_women_1844.csv.gz"), "policyengine"
...
if __name__ == "__main__":
    ...
    d = d0[mask].copy()
    tag = f"{src}_{a.sample}_{a.outcome}" + ("_notyet" if a.notyet else "")
    ...
    for name, df in [("event", E), ("group", G), ("calendar", C), ("simple", S)]:
        df.to_csv(TAB / f"full_{tag}_{name}.csv", index=False)
        print(f"\n## {name}\n" + to_markdown(df, ".4f", index=False))
    A = pd.DataFrame([{"g": g, "t": t, "ATT": v} for (g, t), v in att.items()]); A.to_csv(TAB / f"full_{tag}_attgt.csv", index=False)
    ...
    style_axes(ax); fig.tight_layout(); fig.savefig(FIG / f"full_{tag}_event.png", dpi=200)
    print("wrote", FIG / f"full_{tag}_event.png")
