"""
04_summarise.py -- assemble the full-window results written by
02_csdid.py (via 05_run_full.sh) into paper-style tables and figures.

Outputs (output/):
  tables/summary_simple.md|csv     simple ATT by sample x outcome
  tables/summary_event.md|csv      event-time coefficients, headline samples
  tables/summary_group.md|csv      cohort ATTs, mothers of under-6s
  figures/event_mothers_vs_childless.png
  figures/group_worked.png
  figures/raw_trends_worked.png
"""
import sys
...
def load(kind, s, y, notyet=False):
    p = TAB / f"{s}_{y}{'_notyet' if notyet else ''}_{kind}.csv"
...
T.to_csv(TAB / "summary_simple.csv"); (TAB / "summary_simple.md").write_text(to_markdown(T) + "\n")
print(to_markdown(T))
...
EV.to_csv(TAB / "summary_event.csv"); (TAB / "summary_event.md").write_text(to_markdown(EV) + "\n")
print("\n" + to_markdown(EV))
...
if G is not None:
    ...
    Gt.to_csv(TAB / "summary_group.csv"); (TAB / "summary_group.md").write_text(to_markdown(Gt) + "\n")
    print("\n" + to_markdown(Gt))
...
fig.tight_layout(); fig.savefig(FIG / "event_mothers_vs_childless.png", dpi=200); plt.close(fig)
...
if G is not None:
    ...
    fig.tight_layout(); fig.savefig(FIG / "group_worked.png", dpi=200); plt.close(fig)
...
style_axes(ax); fig.tight_layout(); fig.savefig(FIG / "raw_trends_worked.png", dpi=200); plt.close(fig)
print("\nfigures written")
