...
csdid worked `xvars' [iw = wt], time(year) gvar(gvar) method(drimp) cluster(statefip) wboot rseed(2) notyet
...
collapse (mean) worked inlf employed hours fulltime [pw = n], by(statefip year gvar_year grp)
reshape wide worked inlf employed hours fulltime, i(statefip year gvar_year) j(grp)
gen d_worked = worked1 - worked0
gen d_inlf = inlf1 - inlf0
gen d_employed = employed1 - employed0
gen d_hours = hours1 - hours0
gen gvar = gvar_year
replace gvar = 0 if gvar > 2024
csdid d_worked, ivar(statefip) time(year) gvar(gvar) method(reg) cluster(statefip) wboot rseed(3)
estat event, window(-6 6) estore(rb_ddd_worked)
csdid_plot, title("Triple difference: mothers of under-6s minus childless women") name(ddd, replace)
graph export "$FIG/event_ddd_worked.png", replace width(1600)
...
did_imputation worked statefip year gvar [aw = n], allhorizons pretrends(6) cluster(statefip)
...
eventstudyinteract worked lead6-lead2 lag0-lag6 [aw = n], cohort(gvar) control_cohort(never) ///
    absorb(i.statefip i.year) vce(cluster statefip)
estimates store rb_sa

* de Chaisemartin & D'Haultfoeuille
did_multiplegt_dyn worked statefip year treat, effects(6) placebo(6) cluster(statefip) weight(n)

* Two-way fixed effects for reference (biased under heterogeneous effects)
reghdfe worked treat [aw = n], absorb(statefip year) vce(cluster statefip)
estimates store rb_twfe

*---------------------------------------------------------------- (d) Honest DiD (Rambachan & Roth 2023)
* Uses the CS event-study coefficients stored in 02_csdid.do (cs_worked_event).
estimates restore cs_worked_event
honestdid, pre(1/7) post(9/16) mvec(0(0.01)0.05) coefplot
graph export "$FIG/honestdid_worked.png", replace width(1600)
...
gen gvar = gvar_full
replace gvar = 0 if gvar > 2024
csdid worked `xvars' [iw = wt], time(year) gvar(gvar) method(drimp) cluster(statefip) wboot rseed(4)
...
gen gvar = gvar_year
replace gvar = 0 if gvar > 2024
csdid worked `xvars' [iw = wt], time(year) gvar(gvar) method(drimp) cluster(statefip) wboot rseed(5)
