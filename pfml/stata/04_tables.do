...
estpost tabstat inlf employed hours fulltime age married black hispanic college [aw = wtfinl], ///
    by(ever) statistics(mean sd) columns(statistics)
esttab using "$TAB/t1_descriptives.tex", replace cells("mean(fmt(3)) sd(fmt(3))") ///
    label nonumber booktabs title("Mothers of children under 6, CPS 2000-2024")

* Table 2: CS aggregate effects, four outcomes
esttab cs_inlf_simple cs_employed_simple cs_hours_simple cs_fulltime_simple ///
    using "$TAB/t2_cs_simple.tex", replace se star(* 0.10 ** 0.05 *** 0.01) booktabs ///
    mtitles("In LF" "Employed" "Hours" "Full time") title("Simple ATT, never-treated controls, drimp")

* Table 3: by cohort
esttab cs_inlf_group cs_employed_group using "$TAB/t3_cs_group.tex", replace se ///
    star(* 0.10 ** 0.05 *** 0.01) booktabs mtitles("In LF" "Employed") title("ATT by adoption cohort")

* Table 4: event-study coefficients, main vs robustness
esttab cs_inlf_event rb_notyet rb_fullyear rb_ddd_inlf rb_placebo_childless ///
