...
\begin{table}[H]
\centering
\caption{Means in the year before the event, 73 unit events from 2010: event units and clean controls}
\label{tab:summary}
\small
\input{tables/tab2_summary}
\begin{tablenotes}\footnotesize
\item Weighted means over the twelve months before each event, pooled across the 73 events; event units are the treated county or state remainder, controls the clean control units of that event. Hourly wage among hourly-paid workers in the outgoing rotation groups, earnings-weighted.
\end{tablenotes}
\end{table}
...
The stacked coefficients in Table~\ref{tab:groupssesreg} repeat the pattern within each income group. The only coefficient beyond two standard errors is employed-only among high-income 16--17 year olds, $+0.5$ points (s.e.\ 0.2), a rise; the neither share changes by $-0.3$ (0.5) for low-income and $-0.1$ (0.3) for high-income 16--17 year olds.
...
The stacked coefficients for the wage (Table~\ref{tab:wagereg}) are 5.1 percent (s.e.\ 0.8) at 16--17, 3.0 (0.8) at 18--19 and 3.8 (0.7) pooled, next to the event-study averages of 5.3, 3.0 and 3.8; for weekly earnings they are 5.2 (2.4), 4.4 (1.4) and 4.8 (1.7). The two-way fixed effects elasticity of the teen wage with respect to the minimum, in Appendix~\ref{app:twfe}, is about 0.2 at every age, which for the average event implies gains of 5 to 7 percent. Where the treatment has a real effect the designs agree in sign and roughly in size.
