...
Table~\ref{tab:groupsreg} gives the regression counterparts of Section~\ref{sec:reg}. The stacked difference-in-differences coefficients reproduce the event-study averages: $-0.1$ points (s.e.\ 0.4) for enrolled-and-employed at 16--17 and $+0.4$ (0.5) at 18--19, $-0.2$ (0.3) and $-0.3$ (0.4) for neither, and nothing beyond about one standard error from zero for any share or age. The two-way fixed effects regression disagrees on one share. Its coefficient on the log minimum for neither enrolled nor employed is 0.022 (s.e.\ 0.007) at 16--17 and 0.018 (0.008) at 18--19, which for the average event implies a rise of 0.7 and 0.6 points in the share of teens doing neither, and its coefficient for enrollment at 16--17 is $-0.021$ (0.010), a fall of 0.7 points; the enrolled-and-employed coefficients are negative and marginal. These are the signs and magnitudes of the two-way fixed effects literature \citep{neumark1995a,neumark2019}, and they do not survive the move to clean-control comparisons: the stacked estimates for the same outcomes are $-0.2$ (0.3) and $-0.1$ (0.3), and the fixed-effects-implied effects sit at or beyond the upper ends of their intervals. The difference lies in the variation used. Regression~\eqref{eq:twfe} identifies $\beta$ from the whole 2010--2026 path of the log minimum, the levels and slow schedules of the units that raised repeatedly against the flat path of the federal-floor states, so a differential trend in the share of idle teens between the two groups over sixteen years, of which the recovery from the 2010 peak in the South is one candidate, is attributed to the minimum wage. The event design compares each unit with its controls over a seven-year window around a discrete jump and tests the trend assumption with the pre-event coefficients, which are flat.\footnote{Adding unit-specific linear trends to regression~\eqref{eq:twfe}, the usual remedy, removes the coefficient on neither (0.001, s.e.\ 0.005, at 16--17) but makes the other estimates erratic, for example $-0.11$ (0.05) for employment and $0.30$ (0.03) for the log wage at 16--19, because with a minimum that trends within units the trend terms absorb the treatment path itself \citep{meer2016}. The estimates are in the replication files.}
...
\begin{tablenotes}\footnotesize
\item TWFE: coefficient on the log effective minimum of the teen's unit, regression~\eqref{eq:twfe}, all teens from January 2010, unit and year-month fixed effects, controls for age, sex, race and Hispanic origin, survey weights; multiply by 0.32 for the average event. Stacked DiD: coefficient on post $\times$ treated, regression~\eqref{eq:stacked}, unit-month cells stacked over the 73 events with their clean controls, event-by-unit and event-by-month fixed effects, population weights. Standard errors clustered by state in parentheses. $^{*}$, $^{**}$, $^{***}$: significant at 10, 5 and 1 percent.
\end{tablenotes}
\end{table}
...
The regression counterparts in Table~\ref{tab:groupssesreg} repeat the pattern of Table~\ref{tab:groupsreg} within each income group. The stacked coefficients are within one standard error of zero for every share, age and group. The two-way fixed effects coefficient on neither is positive in both groups, 0.019 (s.e.\ 0.011) for low-income and 0.021 (0.006) for high-income 16--17 year olds, so the fixed-effects result is not a low-income phenomenon that the event design misses; it appears for the teens whose families can pay for school as much as for those whose families cannot, which is not what either channel of Section~\ref{sec:theory} predicts and is what a differential trend common to all teens in the raising units would produce.
...
On the wage the two regression estimators agree with each other and with the event study (Table~\ref{tab:wagereg}). The two-way fixed effects elasticity of the teen hourly wage with respect to the minimum is 0.23 (s.e.\ 0.02) at 16--17, 0.16 (0.02) at 18--19 and 0.19 (0.02) pooled, which for the average event implies gains of 7.3, 5.2 and 6.1 percent; the stacked coefficients are 5.0 (0.8), 3.2 (0.8) and 4.0 (0.7) percent, next to the event-study averages of 5.3, 3.0 and 3.8. Weekly earnings give elasticities of 0.23, 0.15 and 0.19 and stacked gains of 5.4, 4.8 and 5.5 percent. The fixed effects estimates run somewhat above the event-based ones because they use the full level of the schedules, including steps beyond the four-year window, whereas the post-event average dilutes the effect with the pre-increase months of year 0. Where the treatment has a real effect, the designs agree in sign and roughly in size; where the fixed effects regression finds an effect on time allocation that the event design does not, the disagreement is about the design.
