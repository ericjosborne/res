...
forvalues y = 2016/2024 {
    local f "$RAW/nsch/nsch_`y'_topical.dta"
    ...
    rename *, lower
    gen survey_year = `y'
    keep survey_year fipsst fwc stratum hhid sc_age_years sc_sex a1_relation a1_sex a1_age a1_marital a1_employed a1_grade ///
         a1_menthealth a1_physhealth a2_relation a2_sex a2_menthealth a2_physhealth k8q30 k8q31 k8q32 k8q34 k8q35 ///
         k2q01 k6q20 everbreastfed breastfedend_mo_s breastfedend_wk_s fpl_i1-fpl_i6 sc_race_r sc_hispanic_r currcov
...
gen birth_year = survey_year - child_age
gen year = birth_year
...
replace a1_employed = a1_employed == 1 if !missing(a1_employed)
gen a1_educ3 = cond(a1_grade <= 3, 1, cond(inrange(a1_grade, 4, 6), 2, cond(a1_grade >= 7, 3, .)))   // verify codes
...
egen parent_stress = rowmean(k8q31 k8q32 k8q34)
gen child_fairpoor = k2q01 >= 4 if inrange(k2q01, 1, 5)
gen prev_visit = k6q20 == 1 if !missing(k6q20)
gen everbf = everbreastfed == 1 if !missing(everbreastfed)
gen bf_weeks = breastfedend_mo_s * 4.33 + cond(missing(breastfedend_wk_s), 0, breastfedend_wk_s)
